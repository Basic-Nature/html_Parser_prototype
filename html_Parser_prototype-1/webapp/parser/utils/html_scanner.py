...
def parse_html_page(page):
    ...
    logger.info(f"[INFO] Output generated: {output_paths}")
...
def main():
    ...
    with sync_playwright() as playwright:
        ...
        # Detect formats and handle downloads if necessary
        formats = detect_format_from_links(page)
        if formats:
            logger.info(f"[INFO] Detected formats: {formats}")
            # Here you can prompt the user to select formats or auto-confirm
            # For simplicity, we will just log the detected formats

        # Parse the HTML page for election data
