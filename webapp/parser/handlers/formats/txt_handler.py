        ...
        try:
            with open(txt_path, mode="r", encoding=encoding, newline="") as handle:
                ...
                for raw in reader:
                    if not raw:
                        continue
                    clean = { (k or "").strip(): (v if v is not None else "") for k, v in raw.items() }
...
def parse(
    ...
    **kwargs: Any,
    ...
    result = parse_txt_election_results(manual_file, session_id=session_id, coordinator=coordinator)
