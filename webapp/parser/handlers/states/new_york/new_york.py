...
def parse(page: Page, html_context: Optional[dict] = None) -> Tuple[Any, Any, Any, dict]:
    ...
    if not county:
        log_warning("[NY Handler] No county specified in html_context.")
    ...
    try:
        ...
        log_info(f"[NY Handler] Routing to county parser: {module_path}")
        ...
        log_warning(f"[NY Handler] No specific parser implemented for county: '{county}'. Please add it under {module_path}.py")
        return None, None, None, {"error": f"No handler found for NY county: '{county}'"}
    except Exception as e:
        log_error(f"[NY Handler] Error in county parser: {e}")
