...
def scan_and_notify(context):
    ...
    """
    log_info("[BOT] Scanning for new results and sending notifications...")
    new_results = []
    ...
    if new_results:
        ...
        log_info(f"[BOT] Notifications sent for {len(new_results)} new results.")
        return True
    else:
        log_info("[BOT] No new results found for notification.")
        return False
...
def batch_status_report(context):
    ...
    """
    log_info("[BOT] Generating batch status report...")
    report = []
    if context and "contests" in context:
        for contest in context["contests"]:
    ...
    report_text = "\n".join(report)
    if report_text:
        log_info(f"[BOT] Batch Status Report:\n{report_text}")
        # Optionally, send the report via notification
        send_notification("Batch Status Report:\n" + report_text)
        return report_text
    else:
        log_info("[BOT] No contests found for batch status report.")
        return ""
...
def send_notification(message, context=None, email=None):
    ...
    """
    log_info(f"[BOT] Sending notification: {message}")
    ...
    if os.getenv("NOTIFY_EMAILS", "false").lower() == "true":
        ...
        if email_to:
            try:
                ...
                log_info(f"[BOT] Email notification sent to {email_to}")
            except Exception as e:
                log_error(f"[BOT] Failed to send email notification: {e}")
        ...
        if slack_webhook:
            try:
                ...
                if resp.status_code == 200:
                    log_info("[BOT] Slack notification sent.")
                else:
                    log_error(f"[BOT] Slack notification failed: {resp.text}")
            except Exception as e:
                log_error(f"[BOT] Failed to send Slack notification: {e}")
        ...
        if twilio_sid and twilio_token and twilio_from and twilio_to:
            try:
                ...
                if resp.status_code == 201:
                    log_info("[BOT] SMS notification sent.")
                else:
                    log_error(f"[BOT] SMS notification failed: {resp.text}")
            except Exception as e:
                log_error(f"[BOT] Failed to send SMS notification: {e}")
