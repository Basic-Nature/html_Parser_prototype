...
def prompt_url_selection(urls: List[str], processed: Dict[str, Any], prompt_func, output_func):
...
def ai_analyze_results(headers, data, contest_title, metadata):
    if ENABLE_AI_ANALYSIS:
        try:
            ...
            if anomalies:
                logger.error(f"[bold red][AI ALERT][/bold red] Potential anomalies detected: {anomalies}")
...
def orchestrate_url(target_url, processed_info, prompt_func, output_func, session_id=None, cancel_flag=None):
    from .Context_Integration.context_coordinator import ContextCoordinator
    rejected_downloads = set()
    output_func(f"Navigating to: {target_url}")
    ...
    try:
        with sync_playwright() as p:
            ...
            if cancel_flag and cancel_flag.is_set():
                output_func(f"[CANCELLED] Processing stopped for {target_url}")
                return
            if not page:
                output_func(f"[ERROR] Could not open page for {target_url}")
                return
            ...
            if not isinstance(result, tuple) or len(result) != 4:
                output_func("[ERROR] Handler did not return a valid result tuple.")
            ...
            if context.get("batch_mode") and "selected_races" in context:
                try:
                    ...
                    output_func(f"[Batch Mode] Coordinator batch handling failed: {e}")
            ...
            if all([headers, data, contest_title, metadata]):
                ...
                if output_file:
                    if os.path.exists(output_file):
                        output_func(f"[OUTPUT] CSV written to: {output_file}")
                    else:
                        output_func(f"[WARN] Output file path returned but file does not exist: {output_file}")
                else:
                ...
                output_func("Incomplete result structure — skipping CSV write.")
                mark_url_processed(target_url, status="partial")

    except Exception as e:
        output_func(f"[ERROR] Exception while processing {target_url}: {e}")
        mark_url_processed(target_url, status="error")
    finally:
        try:
            if browser:
...
def main(prompt_func=prompt.prompt_input, output_func=logger.info, session_id=None, cancel_flag=None):
    try:
        ...
        selected_urls = prompt_url_selection(urls, processed_info, prompt_func=prompt_func, output_func=output_func)
