    ...
    for folder in ALLOWED_ROOTS:
        for ext in ("*.jsonl", "*.json"):
            for path in glob.glob(os.path.join(folder, ext)):
                ...
                print(f"Importing {path} to table {table_name}...")
                try:
...
def export_postgres_to_json():
    ...
    for table_name in insp.get_table_names():
        ...
        if not is_safe_path(out_path, ALLOWED_ROOTS):
            print(f"[SKIP] Unsafe output path: {out_path}")
            continue
    ...
    for folder in ALLOWED_ROOTS:
        for db_path in glob.glob(os.path.join(folder, "*.db")):
            if not is_safe_path(db_path, ALLOWED_ROOTS):
                print(f"[SKIP] Unsafe path: {db_path}")
                continue
            sqlite_engine = create_sqlite_engine(f"sqlite:///{db_path}")
            insp = inspect(sqlite_engine)
            for table_name in insp.get_table_names():
                print(f"Importing SQLite table {table_name} from {db_path} to PostgreSQL...")
                try:
                    df = pd.read_sql_table(table_name, sqlite_engine)
                    df.to_sql(table_name, engine, if_exists="replace", index=False)
                except Exception as e:
                    print(f"[ERROR] {db_path}:{table_name}: {e}")
        ...
        for db_path in glob.glob(os.path.join(folder, "*.db")):
            ...
            for table_name in insp.get_table_names():
                ...
                if not is_safe_path(out_path, ALLOWED_ROOTS):
                    print(f"[SKIP] Unsafe output path: {out_path}")
                    continue
                print(f"Exporting SQLite table {table_name} from {db_path} to {out_path}...")
                try:
                    df = pd.read_sql_table(table_name, sqlite_engine)
                    df.to_json(out_path, orient="records", lines=True)
                except Exception as e:
