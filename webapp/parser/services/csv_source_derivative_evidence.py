"""Dormant trusted observation for decoded CSV source derivatives.

Transport-neutral and noncanonical. The service fingerprints an already-decoded
CSV representation held by a parser and emits only hashes/counts when an
explicit callback is supplied. It never opens the source file, hashes a path,
...
CONTRACT = "csv_source_derivative_observation_v1"
SURFACE = "CSV_SOURCE_DERIVATIVE"
...
class CsvSourceDerivativeObservationError(RuntimeError):
    """Requested CSV source evidence could not be formed safely."""
...
def _source_identity(value: Any) -> str | None:
    ...
    if not isinstance(value, str) or not _SHA256_RE.fullmatch(value):
        raise CsvSourceDerivativeObservationError(
...
def _header(value: Any) -> str | None:
    ...
    if not isinstance(value, str):
        raise CsvSourceDerivativeObservationError(
            "decoded CSV headers must contain strings or null."
        )
    return value
    ...
    if isinstance(value, (list, tuple)):
        ...
        for item in value:
            if item is not None and not isinstance(item, str):
                raise CsvSourceDerivativeObservationError(
                    "decoded CSV overflow cells must contain strings or null."
                )
            items.append(item)
        return items
    raise CsvSourceDerivativeObservationError(
        "decoded CSV cells must be strings, string lists, or null."
    )
...
def _projection(
    ...
    return {"headers": headers, "rows": rows}, nonempty


def observe_csv_source_derivative_if_requested(
    *,
    ...
    """Emit one hash-only CSV source observation, or remain dormant."""
    if emit_func is None:
        return None
    if not callable(emit_func):
        raise CsvSourceDerivativeObservationError(
            "CSV source derivative observer must be callable."
        )
    if not isinstance(producer, str) or producer not in SUPPORTED_PRODUCERS:
        raise CsvSourceDerivativeObservationError(
            "CSV source derivative producer is unsupported."
        )
    if not isinstance(encoding, str) or not encoding or len(encoding) > 64:
        raise CsvSourceDerivativeObservationError(
            "CSV source derivative encoding is invalid."
        )

    source_identity = _source_identity(source_document_sha256)
    decoded, nonempty = _projection(decoded_headers, decoded_rows)
    ...
    try:
        ...
        raise CsvSourceDerivativeObservationError(
            "CSV source derivative observation callback failed."
        ) from exc
