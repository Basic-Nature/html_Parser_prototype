"""C2G 1.5 contract for the second live typed format-handler seam."""
...
def test_xlsx_primary_path_uses_exactly_one_typed_builder_boundary() -> None:
    assert _call_count(
...
def test_xlsx_primary_typed_call_declares_xlsx_source_type() -> None:
    tree, _ = _function_tree(xlsx_handler.parse_xlsx_election_results)
    calls = [
        node
        for node in ast.walk(tree)
...
def test_xlsx_fallback_parse_remains_legacy_control_path() -> None:
    assert _call_count(xlsx_handler.parse, "build_table_noninteractive") == 1
    assert _call_count(xlsx_handler.parse, "build_table_noninteractive_result") == 0


def test_xlsx_primary_finalization_boundary_remains_single() -> None:
    assert _call_count(
        xlsx_handler.parse_xlsx_election_results,
