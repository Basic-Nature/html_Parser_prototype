...
def api_workflow_v1_claim_second_pass(item_id):
    principal, denied = _workflow_contributor_authority(CAP_DL2_CLAIM)
    if denied is not None:
        return denied
        ...
        return jsonify(
            {
                ...
                "message": "DL2 claim requires exactly expected_row_version.",
            }
        ), 400
    ...
    try:
        ...
        db_session.rollback()
        logger.exception(
            "Unexpected failure while claiming governed Workflow DL2."
        )
        return jsonify(
            {
                "success": False,
...
_WORKFLOW_DL2_SUBMIT_REQUEST_KEYS = _WORKFLOW_DL1_SUBMIT_REQUEST_KEYS


def api_workflow_v1_submit_second_pass(item_id):
    principal, denied = _workflow_contributor_authority(CAP_DL2_SUBMIT)
    if denied is not None:
        return denied
    ...
    if not isinstance(body, dict):
        return jsonify(
            {
                "success": False,
                "error": "workflow_dl2_submit_request_invalid",
            }
        ), 400

    body_keys = frozenset(body.keys())
    if body_keys != _WORKFLOW_DL2_SUBMIT_REQUEST_KEYS:
        return jsonify(
            {
                "success": False,
                "error": "workflow_dl2_submit_request_invalid",
                "missing_keys": sorted(
                    _WORKFLOW_DL2_SUBMIT_REQUEST_KEYS - body_keys
                ),
                "unexpected_keys": sorted(
                    body_keys - _WORKFLOW_DL2_SUBMIT_REQUEST_KEYS
                ),
            }
        ), 400
    ...
    db_session = SessionLocal()
    try:
        payload = submit_second_workflow_pass(
            db_session,
            item_id,
            pass_id=body["pass_id"],
            principal=principal,
        ...
        logger.exception(
            {
                "level": "ERROR",
                "type": "workflow",
                "message": "Second workflow pass submit failed.",
                "session_id": None,
            }
        )
        return jsonify(
            {
                "success": False,
                "error": "workflow_dl2_submit_unavailable",
            }
        ), 503
    finally:
        db_session.close()
...
_WORKFLOW_REVIEWER_QC2_REQUEST_KEYS = _WORKFLOW_REVIEWER_QC1_REQUEST_KEYS


def api_workflow_v1_submit_qc2_review(item_id):
    principal, denied = _workflow_reviewer_authority(CAP_QC2_REVIEW)
    if denied is not None:
        return denied
    if not WORKFLOW_REVIEWER_MUTATIONS_ENABLED:
        return jsonify({
    ...
    if not isinstance(body, dict):
        return jsonify({
            "success": False,
            "error": "workflow_qc2_review_invalid_request",
        }), 400
    body_keys = set(body)
    if body_keys != _WORKFLOW_REVIEWER_QC2_REQUEST_KEYS:
        return jsonify({
            "success": False,
            "error": "workflow_qc2_review_invalid_request",
        }), 400
    ...
    try:
        ...
        db_session.rollback()
        logger.exception("Workflow QC2 review failed unexpectedly.")
        return jsonify({
            "success": False,
            "error": "workflow_qc_review_internal_error",
        }), 503
    finally:
        db_session.close()
