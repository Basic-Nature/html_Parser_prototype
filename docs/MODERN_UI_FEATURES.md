...
All advanced feature styles are in `ballot_lens_modern.css`:
...
- `handle_ballot_lens()` - Accepts `direct_urls` and `batch_mode` parameters
...
- [ ] Batch mode checkbox value included in ballot_lens payload
