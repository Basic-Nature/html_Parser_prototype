...
- TODO (LOW): Replace layout/paint-heavy animations with transform-based animations and limit `will-change` usage to improve rendering performance.
- TODO (LOW): Apply SEO/best-practice fixes — add a meta description, use `rel=preconnect` where helpful, and add `crossorigin` + SRI for CDN scripts when possible.

Generated from `tools/lh-report.json` and compatibility scans (`tools/webhint-report.json`).
...
- Exclusions (examples): webapp/parser/Context_Integration/Context_Library/cache/context_cache.json, docs/todos_medium.md
